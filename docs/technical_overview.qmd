...
SPC is divided into two phases. The data preparation phase relies on scripts that only need to be run once. The second phase involves the user choosing a study area and launching a simulation.
The outputs of the first phase have been uploaded to a dedicated Azure repository so that they can be used directly during the second phase.


The full SPC pipeline comprises the following steps.

- **Phase 1** - Data preparation steps:
    1. The [SPENSER](https://github.com/nismod/microsimulation) model creates a synthetic population with basic demographic information for all of GB.
...
- **Phase 2** - Steps after the user has selected a study area:
    ...
    2. Individuals are assigned a single education destination (if under 16) and several potential retail destinations, according to a local version of the [QUANT](https://github.com/maptube/QUANT_RAMP) model.
    3. Individuals are assigned a single workplace destination (if above 16), according to the method described [here](https://alan-turing-institute.github.io/uatk-spc/modelling_methods.html#commuting-flows).
    4. The population, its activities and an optional lockdown modelling are gathered into a single `.pb` file that can be visualised by the [SPC explorer](https://alan-turing-institute.github.io/uatk-spc/app/).

We now explain how to run each step. The theoretical concepts supporting the modelling are presented [here](https://alan-turing-institute.github.io/uatk-spc/modelling_methods.html).
...
Instructions to run this step from the source code can be found under [Step 1: Curate public data from diverse sources](https://github.com/alan-turing-institute/uatk-spc/tree/main/scripts/data_prep).
...
To understand the methods supporting the creation of these files, we refer the reader to the corresponding section inside the [modelling methods](https://alan-turing-institute.github.io/uatk-spc/modelling_methods.html) section.
...
An individual among those sharing the same 5-year age group (extra details for under 18) and sex is drawn from the participants of the Health Survey (l. 56-72).
...
This join is not spatially differentiated and other matching criteria (ethnicity and nssec8) were retained due to a lack of representativity inside the survey. The BMI field is then added l. 74-89, according to [this method](https://alan-turing-institute.github.io/uatk-spc/modelling_methods.html#bmi-estimation).
...
An individual among those sharing the same 5-year age group, sex and nssec8 category is drawn from the participants of the UK Time Use Survey (l. 111-125).
