...
The generation of the population data by [SPENSER](https://github.com/nismod/microsimulation) and the modelling of trips to schools and retail by [QUANT](https://github.com/maptube/QUANT_RAMP) are detailed in
...
The socio-economic category variable was discarded for the modelling as it is not independent from the other variables, and "mixed" and "other" ethnicities have been merged due to small sample sizes.
...
we simply complete missing values by appoximating them by values of the category that is immediately higher within the SOC hierarchy.
...
based on a combination of the Nomis UK Business Counts 2020 dataset and the Nomis Business register and Employment Survey 2015 (see [Data sources](data_sources.qmd)).
...
(e.g. part time and seasonal contracts are not counted differently than full time contracts, jobseekers or people on maternity leave might report the SIC of their last job).
