...
addIdH <- function(msoa, dataM, dataH) {
  ...
  idh <- paste(dataM[ind], str_pad(sapply(dataH[ind], function(x) {
    which(ref == x)
  }), 4, pad = "0"), sep = "_")
  return(idh)
}
...
addIdP <- function(idh) {
  ...
  idp <- paste(idh, str_pad(unlist(sapply(ref, function(x) {
    1:x
  })), 3, pad = "0"), sep = "_")
  return(idp)
}
...
fitted3 <- function(x, a0, a1, a2, a3) {
  ...
  res <- sapply(x, function(y) {
    coef[1] + coef[2] * y + coef[3] * y * y + coef[4] * y * y * y
  })
  return(unname(res))
}
...
drawIncome <- function(soc, sex, age, fulltime, coefFFTR, coefFPTR, coefMFTR, coefMPTR) {
  ...
  if ((sex == 1) && (fulltime == T)) { # 0 = female, 1 = male, consistent with TUS_HSE dataset
...
fillIncome <- function(idx, data, coefFFTR, coefFPTR, coefMFTR, coefMPTR) {
  pwkst <- data$pwkstat[idx]
  ...
  return(cbind(incomeH))
}

addToData <- function(new, region, coefFFT, coefFPT, coefMFT, coefMPT, cores) {
  ...
  incs <- mcmapply(function(x) {
    fillIncome(x, new, coefFFTR, coefFPTR, coefMFTR, coefMPTR)
  }, seq_len(nrow(new)), mc.cores = cores, mc.set.seed = FALSE)
  new$incomeH <- incs
  return(new)
}
